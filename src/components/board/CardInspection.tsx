...
export function CardInspection({ card, definition, onClose }: { card: CardInstance | null; definition?: CardDefinition; onClose: () => void }) {
  ...
  return <AnimatePresence>{inspectionCard && <motion.div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }} role="dialog" aria-modal="true" aria-label="Inspeccion de carta"><motion.div initial={{ scale: 0.9, y: 12 }} animate={{ scale: 1, y: 0 }}><GameCard card={inspectionCard} definition={definition} size="lg" /></motion.div></motion.div>}</AnimatePresence>;
}
